(function(){
 ...
 var E={
  neutral:{bLY:0,bRY:0,bLR:-2,bRR:2,sqL:.18,sqR:.18,mc:318,mt:321,mb:330,mw:26},
  happy:{bLY:-3,bRY:-3,bLR:-3,bRR:3,sqL:.30,sqR:.30,mc:312,mt:317,mb:336,mw:30},
  sad:{bLY:-1,bRY:-1,bLR:-8,bRR:8,sqL:.20,sqR:.20,mc:330,mt:328,mb:331,mw:20},
  angry:{bLY:4,bRY:4,bLR:8,bRR:-8,sqL:.30,sqR:.30,mc:328,mt:326,mb:329,mw:20},
  surprised:{bLY:-10,bRY:-10,bLR:0,bRR:0,sqL:0,sqR:0,mc:320,mt:309,mb:336,mw:15},
  skeptical:{bLY:3,bRY:-9,bLR:5,bRR:-2,sqL:.26,sqR:.06,mc:322,mt:320,mb:324,mw:22}
 };
 ...
 var lidL=gid('lidL'),lidR=gid('lidR'),bL=gid('browL'),bR=gid('browR'),
     ...
     hookhand=gid('hookhand'),yarn=gid('yarnball'),worksq=gid('worksq');
 ...
 function talk(v){talking=(v===undefined)?!talking:!!v;var b=gid('g-talk');if(b)b.classList.toggle('on',talking);}

 // ---- crocheting (continuous until stopped; she watches her work) ----
 var crocheting=false;
 function crochet(v){
  crocheting=(v===undefined)?!crocheting:!!v;
  [hookhand,yarn,worksq].forEach(function(el){if(el)el.classList.toggle('crocheting',crocheting);});
  var b=gid('g-crochet');if(b)b.classList.toggle('on',crocheting);
  if(crocheting){tgx=-1.5;tgy=GY;}else{tgx=0;tgy=0;}
 }
 ...
 function shake(){gest(headrig,860,function(p){return 'rotate('+(Math.sin(p*Math.PI*4)*(1-p)*5).toFixed(2)+' 340 360)';});}
 ...
 function apply(){
  ...
  lidL.setAttribute('transform','translate(306 254) scale(1 '+sL.toFixed(3)+') translate(-306 -254)');
  lidR.setAttribute('transform','translate(374 254) scale(1 '+sR.toFixed(3)+') translate(-374 -254)');
  bL.setAttribute('transform','translate(0 '+cur.bLY.toFixed(2)+') rotate('+cur.bLR.toFixed(2)+' 306 240)');
  bR.setAttribute('transform','translate(0 '+cur.bRY.toFixed(2)+') rotate('+cur.bRR.toFixed(2)+' 374 240)');
 ...
 // prop action = crocheting, so the viewer's "Toggle prop" works on her too
 function raiseCup(v){crochet(v);}
 var cyc=null,order=['neutral','happy','skeptical','angry','sad','surprised'],ci=0;
 function startCycle(){if(cyc)return;var c=gid('g-cycle');if(c){c.textContent='Auto-cycle: on';c.classList.add('on');}cyc=setInterval(function(){ci=(ci+1)%order.length;setEmotion(order[ci]);},2500);}
 function stopCycle(){if(cyc){clearInterval(cyc);cyc=null;}var c=gid('g-cycle');if(c){c.textContent='Auto-cycle: off';c.classList.remove('on');}}
 ...
 on('g-cycle',function(){cyc?stopCycle():startCycle();});
 on('g-talk',function(){talk();});
 on('g-roll',function(){eyeRoll();});
 on('g-nod',nod);on('g-shake',shake);on('g-shrug',shrug);
 on('g-crochet',function(){crochet();});
 crochet(true); // she crochets by default — it's her gist
 window.coach={setEmotion:setEmotion,raiseCup:raiseCup,crochet:crochet,startCycle:startCycle,stopCycle:stopCycle,
 ...
 window.addEventListener('message',function(ev){var d=ev.data||{};switch(d.coachCmd){
   ...
   case 'raise':raiseCup(d.value);break;case 'crochet':crochet(d.value);break;case 'talk':talk(d.value);break;
   case 'look':look(d.value);break;case 'eyeRoll':eyeRoll();break;case 'nod':nod();break;case 'shake':shake();break;case 'shrug':shrug();break;}});
})();
